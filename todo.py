...
def take_input(command):
    if command not in commands:
        ...
        while True:
            try:
                new_item = add(input("Input title, priority, type, and details: "))
                todo_list.append(new_item)
            except NameError:
                continue
            break
        print("Item added! To see the new item type \"list\"")
    elif command == "help":
        help()
    elif command == "exit":
        exit()
        ...
        while True:
            try:
                print("Input the title of the item you want to change, which type you want to change (title, priority, type, details), and the new name / value")
                item_type, change_value, change_pos = change(input("Input: "))
            except NameError:
                continue
            break
        if item_type == "title":
            todo_list[change_pos].title = change_value
            titles.append(change_value.lower())
        elif item_type == "priority":
            todo_list[change_pos].priority = int(change_value)
        elif item_type == "type":
            todo_list[change_pos].type = change_value
        elif item_type == "details":
            todo_list[change_pos].details = change_value
        print("Changed! To see the new value type \"list\"")
    elif command == "export":
        export()
    elif command == "import":
        import_file()
        ...
        while True:
            try:
                remove_pos = remove(input("Input the title for removal: "))
                break
            except ValueError:
                print("Invalid title")
                continue
        todo_list.remove(todo_list[remove_pos])


def add(info):
    try:
        title, priority, type, details = info.split(", ")
    except ValueError:
        print(
    ...
    if title.strip().lower() in titles:
        ...
        raise NameError
    elif not details or details.strip() == "":
    ...
    return ToDoList(title.strip(), int(priority.strip()), type.strip(), details.strip())
...
def change(user_input):
    ...
    try:
        change_title, change_type, new_value = user_input.split(", ")
    except ValueError:
    ...
    if change_type.lower().strip() == "title":
        ...
        # Type and details have no checks so they get returned together
        return change_type.lower().strip(), new_value.strip(), pos
...
def remove(removal):
        ...
        for i in range(len(todo_list)):
            if str(todo_list[i].title).lower().strip() == removal.lower().strip():
                titles.remove(removal.lower().strip())
                remove_pos = i
